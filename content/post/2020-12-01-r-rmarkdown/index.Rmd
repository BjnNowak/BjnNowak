...
To be consistent with our model, we will use a daily time step data set. There are many weather variables in these dataset, but we will focus on the average daily temperature, which is the input required for our model.    
...
For the rest of the tutorial, we will start from the input table and then progressively calculate the internal variables up to the output variable: the number of leaves. To do so, we will use a combination of functions assembled thanks to the pipe operator (**%>%'**), which allows to perform a sequence of actions. 
...
head(data)
```
In this tutorial, we are interested in corn development. In the northern hemisphere, corn is rarely sown before April 1st. Last step of our data preparation will be to keep only the dates after April 1st (day number = 92). To do so, we will use the filter() function.
...
In the next tutorial, we will see how to analyze the performance of this model (sensitivity analysis, uncertainty analysis, root mean square error...).
...
You will find below a suggestion to code the whole model and estimate potential maximum maize yield from temperature and radiation.
```{r, message=FALSE, warning=FALSE}
...
model_fun <- function(
  ...
  RUE<-2.5    # Radiation use efficiency (gDM.MJ-1)
  # Estimate yield
  model<-data%>%
    dplyr::mutate(
      TT=dplyr::case_when(
